...
This pattern appears throughout the Pony ecosystem. The standard library's `net` package defines `SendToFailure` as a union of `SendToWouldBlock`, `SendToError`, and `SendToNotOpen`. The [ponylang/postgres](https://github.com/ponylang/postgres) driver uses `ClientQueryError` to distinguish query failures. The standard library's `files` package uses `FileErrNo` to represent OS-level file errors.

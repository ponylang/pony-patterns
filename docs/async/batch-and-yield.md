...
You'll see this same structure in Pony's networking layer. When data arrives on a TCP connection, the `on_received` callback returns a `ReadAction`. Returning `KeepReading` causes the read loop to continue delivering data. Returning `YieldReading` causes the loop to stop after this message — it's literally Batch and Yield. The next read will come from a new behavior call, giving the scheduler a chance to run other actors in between.

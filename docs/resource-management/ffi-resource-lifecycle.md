...
For the related case of one-time library initialization and teardown (not per-instance resources), see the [FFI Global Initializer](../creation/ffi-global-initializer.md) pattern. For coordinating shutdown across multiple actors that hold ASIO resources (TCP listeners, timers, signal handlers), see the [Disposable Actor](disposable-actor.md) pattern. The two patterns layer naturally: an actor implements `dispose()` for coordinated shutdown, and inside it a class wraps a C handle with the sentinel-and-finalizer approach described here.

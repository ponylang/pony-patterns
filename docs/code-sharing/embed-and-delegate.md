...
The real payoff shows up in production libraries. Pony's standard library `net` package uses exactly this pattern. Its `TCPConnection` class runs to roughly 2000 lines of connection management logic: socket setup, read/write buffering, backpressure, error handling. The `TCPConnectionActor` trait is about 40 lines of behavior forwarding. A concrete echo server actor built on top of it is around 17 lines. All the complexity lives in the class; the actor just plugs in its application-specific callbacks. [Stallion](https://github.com/ponylang/stallion), a Pony HTTP server, layers on top of the stdlib's TCP connection class, demonstrating that the pattern composes across library boundaries.
